  ...
  return (
    <div className="min-h-screen bg-[#F7F5F2]">
      <div className="max-w-3xl mx-auto py-12">
        <h1 className="text-3xl font-bold mb-8 text-[#260B01]">
          Rezervasyonlarım
        </h1>
        {loading ? (
          ...
          <ul className="space-y-4">
            {reservations.map((r) => (
              ...
              >
                <span>
                  Ev: <b>{r.houseLocation || r.houseName || r.houseId}</b>{" "}
                  <br />
                  Tarih: {new Date(r.startDate).toLocaleDateString()} -{" "}
                  {new Date(r.endDate).toLocaleDateString()} <br />
                  Tutar: ₺{r.totalPrice}
                </span>
                <div className="flex flex-col gap-2 items-end">
                  ...
                  <button
                    ...
                    onClick={() =>
                      router.push(`/add-review?houseId=${r.houseId}`)
                    }
                  >
                    Yorum Ekle
                  </button>
                    ...
                    onClick={async () => {
                      if (
                        ...
                        const resp = await fetch(
                          `http://localhost:5254/api/reservations/${r.id}`,
                          {
                            method: "DELETE",
                          }
                        );
